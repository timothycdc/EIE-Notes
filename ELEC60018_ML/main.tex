
\documentclass{report}
    \title{EEE ML notes}
    \author{Timothy Chung}
    \date{21/10/23}

\usepackage[a4paper, total={7in, 10in}]{geometry}
\setlength\parindent{0pt}
\input{common.tex}
\newcommand{\infer}[2]{\cfrac{#1}{#2}}
\newcommand{\infers}[3]{\cfrac{#1}{#2}(#3)}

\newcommand{\ninfer}[3]{(\text{#1}):\cfrac{#2}{#3}}
\newcommand{\ninfers}[4]{(\text{#1}):\cfrac{#2}{#3}(#4)}
\newcommand{\ainfer}[3]{\cfrac{#2}{#3}(\text{#1})}

\newcommand{\mllet}[2]{\text{let } #1 \text{ in } #2}
\newcommand{\mlfix}[2]{\text{fix } #1 . #2}

\newcommand{\hc}{\mathcal{H}}


\begin{document}
\include{titlepage/titlepage}

\begin{titlepage}
    \centering
    \vspace*{1cm}
    \Huge
    \textbf{ELEC60019 Machine Learning} \\
    \vspace{1cm}
    \Large
    Timothy Chung \\
    \vspace{1cm}
    Autumn 2023 \\
    \vfill
\end{titlepage}

\setcounter{tocdepth}{1}
\tableofcontents
\newpage

% \input{recursion}
\input{1_ml_intro}
\input{2_learning}
\input{3_extending_linearity}
\input{4_optimisation_and_learning}
\input{5_geometric_classification}
\input{6_unsupervised_learning_dimreduct}
\input{7_advanced_clustering}
\end{document}

